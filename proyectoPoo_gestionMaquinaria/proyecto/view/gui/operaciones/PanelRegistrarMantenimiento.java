...
public class PanelRegistrarMantenimiento extends JPanel{
    ...
    private void registrarMantenimiento() {
        String codigoMaquinaria = codigo.getText().trim();
        String costoMantenimiento = costo.getText().trim();
        String fechaMantenimiento = fecha.getText().trim();
        String descripcionMantenimiento = descripcion.getText().trim();

        if (codigoMaquinaria.isEmpty() || costoMantenimiento.isEmpty() || fechaMantenimiento.isEmpty() || descripcionMantenimiento.isEmpty()) {
            DialogoMensaje.mostrarAdvertencia(this, "Por favor, complete todos los campos.");
            return;
        }
        camResultado.setText("Mantenimiento registrado exitosamente para la maquinaria con código '" + codigoMaquinaria + "'.");
    }
}